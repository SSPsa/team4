...
<script>
    ...
    /*线路管理-停用*/
    function member_stopyState(obj,id,yState){
    ...
    /*线路管理-启用*/
    function member_stopyStop(obj,id,yState){
    ...
    /*杆塔管理-停用*/
    function member_stoptState(obj,id,tState){
        layer.confirm('确认要停用吗？',function(index){
            $.ajax({
                type:"GET",
                ...
                success:function(data){
                    if(data.delResult == "true"){//删除成功：移除删除行
                        //发异步删除数据
                        $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="member_stopState(this,id,tState)" href="javascript:;" title="已启用"><i class="layui-icon">&#xe601;</i></a>');
                        $(obj).parents("tr").find(".td-status").html('<span class="layui-btn layui-btn-normal layui-btn-mini" >停用</span>');
                        $(obj).remove();
                        layer.msg('已停用!',{icon: 6,time:1000});
                        window.history.go(0);
    ...
    /*杆塔管理-启用*/
    function member_stoptStop(obj,id,tState){
        layer.confirm('确认要启用吗？',function(index){
            //发异步把用户状态进行更改
            $.ajax({
                type:"GET",
                ...
                success:function(data){
                    if(data.delResult == "true"){//删除成功：移除删除行
                        //发异步删除数据
                        $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="member_stopStop(this,id,tState)" href="javascript:;" title="停用"><i class="layui-icon">&#xe601;</i></a>');
                        $(obj).parents("tr").find(".td-status").html('<span class="layui-btn layui-btn-normal layui-btn-mini">已启动</span>');
                        $(obj).remove();
                        layer.msg('已启用!',{icon: 6,time:1000});
                        window.history.go(0);
